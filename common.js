...
/* ===========================================================
 * 4. Ideen-Wizard  –  jetzt mit dauerhafter Speicherung
 * =========================================================== */
function initIdeenWizard(){
  const feld = $('themenfeld');
  if(!feld) return;                     // nur auf idee.html vorhanden
  ...
  const boosters=[/* …wie gehabt… */];

  /* ---------- Initialer Render ---------- */
  if(phase>1){          // Thema anzeigen, wenn schon vorhanden
    $('themenfeld-display').textContent = thema;
    feld.value = thema;
  }
  if(phase===2){
    renderIdeas();
    tick();             // Timer weiterlaufen lassen
  }
  if(phase===3){
    finishPhase(true);  // true = nur DOM aufbauen, nichts speichern
  }
  step(phase);          // richtige Ansicht aktivieren
  ...
  on($('start-btn'),   'click',start);
  on($('add-idee-btn'),'click',addIdea);
  on($('booster-btn'), 'click',()=>$('creativity-booster').textContent=
      boosters[Math.random()*boosters.length|0]);
  on($('finish-btn'),  'click',finishPhase);
  on($('generate-btn'),'click',generatePrompt);
  on($('copy-btn'),    'click',()=>navigator.clipboard
      .writeText($('generated-prompt').textContent));
  on($('restart-btn'), 'click',()=>{localStorage.removeItem(KEY);location.reload();});
  ...
  function tick(){
    $('timer').textContent=`⏰ Zeit: ${Math.floor(rest/60)}:${String(rest%60).padStart(2,'0')}`;
    if(rest===0) return finishPhase();
    if(rest--===60) $('timer').classList.add('warning');
    save();
    timerId = setTimeout(tick,1000);
  ...
  function addIdea(){
    const text = $('neue-idee').value.trim();
    if(!text) return;
    ideen.push(text); $('neue-idee').value='';
    renderIdeas(); save();
  }
  ...
  function renderIdeas(){
    ...
    $('ideas-container').innerHTML   = ideen
      .map((t,i)=>`<div class="idea-item">${i+1}. ${t}</div>`).join('');
  }
  ...
  function finishPhase(onlyRender=false){
    ...
    step(3);
    $('themenfeld-final').textContent = thema;
    $('final-ideas-list').innerHTML = ideen.map((t,i)=>`
      <div class="idea-item ${sel.includes(i)?'selected':''}" data-i="${i}">
        <input type="checkbox" ${sel.includes(i)?'checked':''}> ${t}
      </div>`).join('');
    document.querySelectorAll('#final-ideas-list .idea-item').forEach(div=>
      on(div,'click',()=>toggleIdea(+div.dataset.i,div)));
    if(!onlyRender) save();
  }
  ...
  function toggleIdea(i,div){
    ...
    const cb  = div.querySelector('input');
    if(idx>-1){ sel.splice(idx,1); div.classList.remove('selected'); cb.checked=false; }
    else if(sel.length<3){ sel.push(i); div.classList.add('selected'); cb.checked=true; }
    else alert('Du kannst maximal 3 Ideen auswählen!');
